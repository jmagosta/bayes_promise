...
*Note, you need to install the command line version of `JAGS` for `rjags` to function. For MacOS [see](https://letmegooglethat.com/?q=Does+RJAGS+run+on+the+Mac%3F).*
...
Note that we are presenting only the second part of the a typical meta analysis. The fist part, a systematic search of the literature to decide what studies to include in the analysis that is not included here, would deserve its own workshop.
...
The data comes from Table 7.1 of the text (page 142), which summarizes the results of thirty-four clinical trials. The column headings are:
